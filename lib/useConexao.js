...
export function useConexao(consultorId) {
  ...
  useEffect(() => {
    ...
    function handleDbBloqueado() {
      setUltimoResultado({
        enviados: 0,
        falhas: 1,
        erros: ["Outra aba antiga do RASTRO está aberta. Feche as outras abas e tente salvar novamente."],
      });
    }
    ...
    window.addEventListener("rastro-db-bloqueado", handleDbBloqueado);
    ...
    return () => {
      ...
      window.removeEventListener("rastro-db-bloqueado", handleDbBloqueado);
