...
export function useDadosRebanho(consultorId, clienteId) {
  ...
  const carregarTudo = useCallback(async () => {
    ...
    try {
      const [{ data: a }, { data: l }, { data: lt }, { data: f }, { data: m }, { data: c }, { data: ab }] = await Promise.all([
        ...
        supabase.from("rebanho_cochos").select("*").eq("cliente_id", clienteId).eq("ativo", true).order("nome"),
        supabase.from("rebanho_abastecimentos_cochos").select("*").eq("cliente_id", clienteId).order("data_abastecimento", { ascending: false }).limit(300),
      ]);
      ...
      const pendentesAbastecimento = abastecimentosLocais.filter((item) => (
        item.sincronizado === false && !(ab || []).some((servidor) => servidor.client_uuid === item.client_uuid)
      ));
      ...
      const [a, l, lt, c, ab] = await Promise.all([
