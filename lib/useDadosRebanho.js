...
import { salvarPendenteLocal, salvarCacheLocal, listarCacheLocal, gerarIdLocal, excluirRegistroLocal } from "./db";
...
export function useDadosRebanho(consultorId, clienteId) {
  ...
  async function excluirMovimentacao(movimentacao) {
    ...
    setMovimentacoes((prev) => prev.filter((item) => (
  ...
  async function registrarProcedimentosEmLote(animaisIds, dados) {
