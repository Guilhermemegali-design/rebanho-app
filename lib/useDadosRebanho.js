...
export function useDadosRebanho(consultorId, clienteId, fazendaId) {
  ...
  const carregarTudo = useCallback(async () => {
    ...
    try {
      ...
      Promise.all([
        salvarCacheLocal("animais", listaAnimais),
        salvarCacheLocal("locais", l || []),
        salvarCacheLocal("lotes", lt || []),
        salvarCacheLocal("cochos", c || []),
        mapa ? salvarCacheLocal("mapas", [mapa]) : Promise.resolve(),
  ...
  async function atualizarLote(id, mudancas) {
    ...
    if ("local_id" in mudancas) {
      const { error: erroAnimais } = await supabase
        .from("rebanho_animais")
        .update({ local_atual_id: mudancas.local_id, atualizado_em: new Date().toISOString() })
        .eq("lote_atual_id", id);
      if (erroAnimais) throw erroAnimais;
      setAnimais((prev) => prev.map((animal) => (
        animal.lote_atual_id === id ? { ...animal, local_atual_id: mudancas.local_id } : animal
      )));
    }
    return data;
  }
