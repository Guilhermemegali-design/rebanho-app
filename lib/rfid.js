...
export function extrairTagRfid(texto) {
  ...
  // cabeçalho + EID ISO (15) + data/hora (8). O tamanho do cabeçalho pode
  // variar; por isso contamos a partir do final, removendo primeiro os 8
  // dígitos variáveis de data/hora. Assim nunca confundimos o relógio do
  // leitor com a identificação do animal.
  if (blocos.length === 1 && blocos[0].length >= 23) {
    const eidAntesDaDataHora = blocos[0].slice(-23, -8);
    return ultimosOitoDigitos(eidAntesDaDataHora);
  }
